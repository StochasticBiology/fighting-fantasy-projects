...
typedef struct {
  ...
  int as_bonus;                    // attack strength bonus
  int skill_buffer;                // skill buffer
...
void dSkill(Player *Hero, int d)
{
  ...
  if(Hero->skill + d <= Hero->i_skill) Hero->skill += d;
    ...
    {
      diff = d-(Hero->i_skill-Hero->skill);
      Hero->skill = Hero->i_skill;

      if(AS_BONUS)
	Hero->as_bonus += diff;
      else if(SKILL_BUFFER)
	Hero->skill_buffer += diff;

    }
  if(SKILL_BUFFER && Hero->skill < Hero->i_skill)
    {
      if(Hero->skill + Hero->skill_buffer < Hero->i_skill)
	{
	  Hero->skill += Hero->skill_buffer;
	  Hero->skill_buffer = 0;
	}
      else
	{
	  Hero->skill_buffer -= (Hero->i_skill - Hero->skill);
	  Hero->skill = Hero->i_skill;
	}
    }	  
}
...
int main(void)
{
  ...
  for(i = 0; i < NSIM; i++)
    {
      ...
      Hero.skill_buffer = 0; Hero.as_bonus = 0;
      Hero.food = 5; 
      Hero.died = 0;
      Hero.potion = 0;
  ...
  if(AS_BONUS)
    printf("Cheating with Attack Strength bonus\n");
  if(SKILL_BUFFER)
    printf("Cheating with Skill buffer\n");
  if(LUCK_POTION)
    printf("Cheating with Luck potion\n");
