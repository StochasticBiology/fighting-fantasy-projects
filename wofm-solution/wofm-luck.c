...
int main(void)
{
  ...
  for(i = 0; i < NSIM; i++)
    {
      ...
      Hero.i_skill = Hero.skill = D6()+6; 
      Hero.i_stamina = Hero.stamina = D6()+D6()+12; 
      Hero.i_luck = Hero.luck = D6()+6; 
      Hero.food = 10; 
      Hero.died = 0;
      Hero.potion = 2;
      F.shield = 0; 
  ...
  printf("Overall success probability %.2f%%\n", 100.*(1.- (double)dead / NSIM));
  if(LUCK)
    printf("... using Luck in battles\n");
  else
    printf("... not using Luck in battles\n");
  for(i = 0; i < NSTAGE; i++)
    {
      if(deadhist[i])
